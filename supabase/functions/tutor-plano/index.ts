...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Buscar cursos relevantes na CEFIS (Catálogo local primeiro)
    let coursesList = [];
    try {
      const searchResponse = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/cefis-search`, {
        ...
        body: JSON.stringify({ query: perfil.objetivo, limit: 20 }),
      });
      
      if (searchResponse.ok) {
        const searchResult = await searchResponse.json();
        coursesList = searchResult || [];
      }
    } catch (e) {
      console.error("Erro ao buscar no catálogo local:", e);
    }

    // Fallback para API externa se local estiver vazio
    if (coursesList.length < 8) {
      ...
      if (cefisResponse.ok) {
        ...
        const apiCourses = cefisResult.data || [];
        
        const existingIds = new Set(coursesList.map((c: any) => c.course_id || c.id));
